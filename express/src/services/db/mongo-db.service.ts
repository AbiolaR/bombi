...
import { PocketBookConfig as PocketBookConfigClass } from "../../models/db/mongodb/pocketbook-config.model";
import { User, UserMethods } from "../../models/db/mongodb/user.model";
...
const PocketBookCloudConfig = new Schema({
    accessToken: { type: String, required: true },
    refreshToken: { type: String, required: true }
});

const PocketBookConfig = new Schema({
    cloudConfig: PocketBookCloudConfig,
    sendToEmail: String 
});

const GoogleCredentials = new Schema({
...
/*UserSchema.method('save', async function() {
...
export const findUserByHash = async (hash: string) => {
    return await User.findOne({ passwordResetHash: hash });
}

export const findUserByUsernameAndCode = async (username: string, code: string) => {
    return await User.findOne({ username: username, passwordResetCode: code });
}

export const findUserByGoogleCredentialState = async (state: string) => {
    return await User.findOne({ 'googleCredentials.state': state });
}
